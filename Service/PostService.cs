...
namespace Blogs.Service
{
    public class PostService : IPostService
    {
        ...
        public PostService(IConfiguration config, IHttpContextAccessor httpContextAccessor)
        {
            _config = config;
            _httpContextAccessor = httpContextAccessor;
        }

        public async Task<(bool Success, string Message, List<Post> Posts)> Posts()
        {
    ...
    ", conn);
        ...
        public async Task<(bool Success, string Message)> NewPost(string title, string content, string tags, IFormFile? image)
        {
            var httpContext = _httpContextAccessor.HttpContext;
            ...
            try
            {
                ...
                return (true, "Пост успешно добавлен");
            }
            catch (Exception ex)
            {
                await transaction.RollbackAsync();
                Console.WriteLine("Ошибка при добавлении поста: " + ex.Message);
                return (false, "Ошибка сервера при добавлении поста 500.");
            }
        }

        public async Task<(bool Success, string Message, object Data)> LikePost([FromBody] LikePostRequest request)
        {
            if (!int.TryParse(request.PostId, out var pid))
                return (false, "Неверный postId", null);

            var httpContext = _httpContextAccessor.HttpContext;

            var username = request.Username ?? httpContext.User.Identity?.Name;
            if (string.IsNullOrWhiteSpace(username))
                return (false, "Пользователь не зарегестрирован", null);
            ...
            if (userId == null) return (false, "Пользователь не найден", null);
            ...
            return (true, "Пост лайкнут", new { liked = !liked, likeCount });
        }
    }
}
